...
echo "<form action=wa.php method=post>";
...
//select all employees who have day off
$select_dayoff = "SELECT * FROM dayoff where date='$pickdate' ";
...
$select_employee = "SELECT DISTINCT fname,lname FROM employees where (categories='$rr' )";
...
echo "<form action=wa.php method=post style>";
...
$employeename=$row['employeename'];
 ...
 $AddQuery = "INSERT INTO orders (date,employee-name) VALUES ('$date','$employeename')";
    mysqli_query($db,$AddQuery);
...
<br> <input id="back" type=submit   name=goback value='<< back'  />
</form>
</div>
</fieldset>
<?php
...
$ending_time = strtotime("+15 minutes",strtotime($time));
